...
TEST_CORS_BLOCK() {
  ...
  local code
  code=$(curl -s -o /dev/null -w "%{http_code}" -H "Origin: https://evil.example" "$BASE/api/health")
...
EOF
  local code
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/vendors/import.csv" -H "x-user-level: 2" -H "x-dry-run: 1" --data-binary @"$tmpcsv")
  [ "$code" = "200" ] && ok || no
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/vendors/import.csv" -H "x-user-level: 2" --data-binary @"$tmpcsv")
  [ "$code" = "200" ] && ok || no
...
TEST_PAYMENTS() {
  ...
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/payments/$id/proof" -H "Content-Type: application/json" -H "x-user-level: 2" --data '{"proof_url":"https://example.com/proof.jpg"}')
  [ "$code" = "403" ] && ok || no
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/payments/$id/proof" -H "Content-Type: application/json" -H "x-user-level: 1" --data '{"proof_url":"https://example.com/proof.jpg"}')
  [ "$code" = "200" ] && ok || no
  code=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/payments/$id/mark-done" -H "x-user-level: 1")
  [ "$code" = "200" ] && ok || no
}
